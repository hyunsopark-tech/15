...
import { RefreshCw, Clock, Stethoscope, FileText } from "lucide-react";
...
const COLOR_MAP = {
  ...
  red: {
    tab: "border-red-500 text-red-700 bg-red-50",
    inactive: "border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300",
...
export default function WorkflowTabs() {
  ...
  const tabPatients = mockPatients.filter((p) => p.workflow === activeTab);
  const activeTabConfig = TABS.find((t) => t.id === activeTab)!;

  const criticalCount = tabPatients.filter((p) => p.priority === "critical").length;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white border-b border-slate-200 px-5 flex-shrink-0">
        <div className="flex items-end gap-0">
          {TABS.map((tab) => {
            const count = mockPatients.filter((p) => p.workflow === tab.id).length;
